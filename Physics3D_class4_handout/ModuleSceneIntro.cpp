...
bool ModuleSceneIntro::Start()
{
	...
	// TODO 2: Chain few N spheres together to form a snak

	for (uint i = 0; i < MAX_SNAKE; ++i) {
		
		s_snake[i] = Sphere(2);
		s_snake[i].SetPos(4*i, 0,0);
		
		pb_snake[i] = App->physics->AddBody(s_snake[i]);
		
		if (i>0 && i<MAX_SNAKE)
		{
			App->physics->AddConstraintP2P(*pb_snake[i - 1], *pb_snake[i], { 2,0,0 }, {-2,0,0});
		}
